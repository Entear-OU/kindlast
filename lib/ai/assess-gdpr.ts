...
export async function assessGDPRCompliance(profile: BusinessProfile): Promise<AssessmentResult> {
  const { object } = await generateObject({
    model: google('gemini-2.5-flash'),
    schema: AssessmentResultSchema,
    system: `You are an expert EU data protection consultant specializing in GDPR compliance for small and medium-sized enterprises. You assess businesses against the full scope of the General Data Protection Regulation (EU) 2016/679.
