...
const provider = createResendProvider({ apiKey: 're_test', from: 'Kindlast <noreply@kindlast.com>' })
const message = { to: 'founder@example.com', subject: 'Hi', html: '<p>Hi</p>', text: 'Hi' }
...
describe('createResendProvider (ENT-76)', () => {
  ...
  it('omits text when not provided', async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).not.toHaveProperty('text')
  })
