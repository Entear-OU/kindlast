import type { Agent } from '@/lib/agents/catalog'
...
export function AgentProfile({ agent }: { agent: Agent }) {
  return (
    <article className="space-y-8">
      ...
      {agent.skill ? (
        <section>
          <SectionHeading>The skill it runs</SectionHeading>
          {/* Name and version together, because that pair is what a run
              records. A version on its own says nothing and a name on its own
              is not something anybody could reproduce a run from. */}
          <p className="mt-3 font-mono text-[13px] text-foreground">
            {agent.skill.name}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Version {agent.skill.version}. Every run records both, so a finding
            can be traced back to what was asked and how.
          </p>
