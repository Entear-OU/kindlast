...
| `fetch-evidence-for-every-connection` | Hourly (`KINDLAST_FETCH_RELAY_INTERVAL`) | Asks core-api which connected systems have evidence older than a day, per granted read-only tool, and starts one `fetch/{connection id}/{tool}` workflow per pair. Each fetch goes through the gateway exactly as one from the Integrations page does (egress allow-list, tool policy, rate limit), runs on the consent of the person who connected the system, and deposits what came back into `org_evidence` with the fetch recorded in `integration_fetches` whatever the outcome. This is what fills the memory the Watcher's `read_evidence` tool reads. Pausing this schedule is the off switch for unattended fetching; a deployment with no gateway fetches nothing and this relay reports empty ticks. |
...
| `KINDLAST_FETCH_RELAY_INTERVAL` | `1h` | How often the relay asks which connections have evidence that has gone stale. Not how often a customer is dialled: each granted tool is fetched at most once a day, and that bound is core-api's, deliberately out of any caller's reach. A Go duration. |
